...
### 오류 코드
- 정확도는 모두 통과했으나, 효율성에서 모두 시간초과
- 코드 변수가 헷갈리거나, 조건문이 잘못되었을 걸로 추측
```
function solution(maps) {
    var answer = 0;
    const m = maps.length - 1, n = maps[0].length - 1;
    const cases = [[0, -1], [-1, 0], [0, 1], [1, 0]]
    ...
    const bfs = () => {
        ...
        while (queue.length > 0) {
            ...
            if (r === m && c === n) return d
             ...
             for (var [dy, dx] of cases) {
                const i = r + dy, j = c + dx;
                
                if (0 <= i && i <= m && 0 <= j && j <= n && maps[i][j] === 1) {
                    queue.push([i, j, d + 1])
                    maps[r][c] = 0
                }
            }
...
## 참고
[[프로그래머스 JavaScript] 게임 맵 최단거리](https://velog.io/@eldoradodo/%ED%94%84%EB%A1%9C%EA%B7%B8%EB%9E%98%EB%A8%B8%EC%8A%A4-JavaScript-%EA%B2%8C%EC%9E%84-%EB%A7%B5-%EC%B5%9C%EB%8B%A8%EA%B1%B0%EB%A6%AC-a3l8xukk)